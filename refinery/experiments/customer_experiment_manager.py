...
from ..core.models import Hypothesis, FileChange
...
class CustomerExperimentManager:
    ...
    def validate_change_path(self, rel_path: str, allowed_roots: Tuple[str, ...] = ("prompts/",)) -> None:
        ...
        if '..' in rel_path:
            raise ValueError(f"Invalid path: {rel_path} (parent directory references not allowed)")
        ...
        raise ValueError(f"Path outside allowed directories: {rel_path} (allowed: {allowed_roots})")
    
    def backup_and_deploy(self, src_dir: Path, files: List[str], backup_id: Optional[str] = None) -> str:
        """Backup existing files then deploy new versions (prevent hard-to-undo demo-killer)."""
        if backup_id is None:
            backup_id = os.environ.get("RUN_ID", f"manual_{int(datetime.utcnow().timestamp())}")
        ...
        logger.info(f"Successfully deployed {len(deployed_files)} files, backed up {len(backup_files)} files")
        return backup_id
