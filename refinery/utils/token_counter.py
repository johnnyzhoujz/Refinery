...
from ..core.models import Trace, TraceRun
...
class TokenCounter:
    ...
    def count_trace_raw(self, trace: Trace) -> Dict[str, Any]:
        ...
        for run in trace.runs:
            ...
            metadata_tokens = self.count_text(f"{run.id} {run.name} {run.run_type.value}")
            
            total_run_tokens = inputs_tokens + outputs_tokens + error_tokens + metadata_tokens
        ...
        stats["largest_runs"] = sorted(run_tokens, key=lambda x: x["tokens"], reverse=True)[:5]
