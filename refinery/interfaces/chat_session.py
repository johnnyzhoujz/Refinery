...
async def run_chat_session(
    interface: BaseChatInterface, codebase: str = ".", default_project: str = "default"
):
    """
    ...
    try:
        ...
        total_files = len(project_context.get("prompt_files", [])) + len(
            project_context.get("eval_files", [])
        )
        ...
        def handle_progress(event_type: str, payload: dict) -> None:
            try:
                ...
                if event_type == "analysis_started":
                    ...
                    log(f"[red]Stage 2 failed: {payload.get('error')}[/red]")
                    ...
                    log(f"[red]Stage 3 failed: {payload.get('error')}[/red]")
                    ...
                    log(f"[red]Stage 4 failed: {payload.get('error')}[/red]")
                ...
                if config.debug:
                    interface.console.log(
                        f"[red]Progress callback error for {event_type}[/red]"
                    )
        ...
        await interface.show_progress(
            "📊 Analyzing trace execution flow... (this may take a minute)"
        )

        # Use Rich status spinner during the long analysis operation
        with interface.console.status(
            "[bold blue]🔬 Analyzing trace data...[/bold blue]", spinner="dots"
        ):
        ...
        if await interface.ask_yes_no("Generate improved prompts using GPT-5?"):
            ...
            try:
                ...
                if hypotheses:
                    ...
                    from ..experiments.customer_experiment_manager import (
                    ...
                    await interface.show_error(
                        "Failed to generate hypothesis. Please try again."
                    )
