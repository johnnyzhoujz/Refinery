...
from ..core.models import CompleteAnalysis, Diagnosis, GapAnalysis, TraceAnalysis
...
class ChatInterface(BaseChatInterface):
    ...
    async def get_project_name(self, default: str = "default") -> str:
        ...
        project = self.console.input(
            f"[bold cyan]Project name[/bold cyan] (default: {default}): "
        ).strip()
    ...
    async def confirm_action(self, message: str) -> bool:
        ...
        response = (
            self.console.input(f"[bold yellow]{message}[/bold yellow] (y/n): ")
            .strip()
            .lower()
        )
...
Let's get started!
    ...
    async def show_diagnosis(self, diagnosis: Diagnosis):
        ...
        if diagnosis.detailed_analysis:
            analysis_preview = (
                diagnosis.detailed_analysis[:200] + "..."
                if len(diagnosis.detailed_analysis) > 200
                else diagnosis.detailed_analysis
            )
            diagnosis_text += f"\n**Detailed Analysis:**\n{analysis_preview}\n"
        ...
        if trace_analysis.execution_flow:
            ...
            if isinstance(trace_analysis.execution_flow, str):
                ...
                for i, step in enumerate(
                    trace_analysis.execution_flow[:5], 1
                ):  # Show first 5 steps
    ...
    async def show_gap_analysis(self, gap_analysis: GapAnalysis):
        ...
        self.console.print(
            Panel(
                Markdown(gap_text.strip()), title="Gap Analysis", border_style="magenta"
            )
        )
    ...
    async def show_recommendations(self, diagnosis: Diagnosis):
        ...
        if diagnosis.remediations:
            ...
            for remediation in diagnosis.remediations[:5]:  # Show top 5 remediations
                ...
                recommendations_text += (
                    f"**Priority: {priority}** | **Effort: {effort.title()}**\n"
                )
        ...
        if diagnosis.top_findings:
            ...
            for finding in diagnosis.top_findings[:3]:  # Show top 3 findings
                ...
                recommendations_text += (
                    f"• {finding_text} *(Confidence: {confidence})*\n"
                )
    ...
    async def ask_yes_no(self, message: str) -> bool:
        """Ask user a yes/no question."""
        response = (
            self.console.input(f"\n[bold cyan]{message}[/bold cyan] (y/n): ")
            .strip()
            .lower()
        )
