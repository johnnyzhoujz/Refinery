...
PROMPT_PATTERNS = [
    '*prompt*', '*template*', 'system_*', 'user_*', 'instruction*'
]

EVAL_PATTERNS = [
    '*eval*', '*test*', '*validation*', '*assess*'
]

CONFIG_PATTERNS = [
    'config*', 'setting*', '.env', '*.yaml', '*.json'
]
...
def _estimate_role(file_path: str, content: str) -> str:
    ...
    # For configs
        ...
        if any(term in content_lower for term in ['model', 'llm', 'openai', 'anthropic', 'gpt']):
            return 'model_config'
        return 'configuration'
...
def _is_model_config(content: str) -> bool:
    ...
    model_keywords = ['model', 'llm', 'openai', 'anthropic', 'gpt', 'claude', 
...
def _classify_file_type(file_path: str, content: str) -> str:
    ...
    if any(pattern in filename for pattern in ['config', 'setting', '.env']) or \
       filename.endswith(('.yaml', '.yml', '.json', '.toml')):
        return 'config'
...
def _generate_summary(context: SimpleAgentContext) -> str:
    ...
    model_configs = sum(1 for info in context.config_files.values() 
                       if info.estimated_role == 'model_config')
    if model_configs:
        summary_parts.append(f"  - Model configurations: {model_configs}")
    
    # Variable usage
    files_with_vars = sum(1 for info in list(context.prompt_files.values()) + 
...
async def build_simple_context(codebase_path: str) -> SimpleAgentContext:
    ...
    logger.info(f"Found {len(prompt_files)} prompt files, {len(eval_files)} eval files, "
