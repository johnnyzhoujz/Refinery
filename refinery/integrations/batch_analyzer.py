...
class BatchTraceAnalyzer:
    ...
    async def analyze_trace_holistic(
        ...
        """
        logger.info(
            "Starting holistic trace analysis",
            trace_id=trace.trace_id,
            runs_count=len(trace.runs),
        )

        # Step 1: Upload trace as file
        trace_file_id = await self._upload_trace_file(
            trace, prompt_contents, eval_contents
        )

        # Step 2: Create batch analysis request
        batch_id = await self._submit_batch_analysis(trace_file_id, expectation, model)
    ...
    async def _upload_trace_file(
        self, trace: Trace, prompt_contents: dict = None, eval_contents: dict = None
    ) -> str:
    ...
    async def _submit_batch_analysis(
        self, trace_file_id: str, expectation: DomainExpertExpectation, model: str
    ) -> str:
...
""",
                ...
                {"type": "file", "file_id": trace_file_id},
            ],
        ...
        while True:
            ...
            if status == "completed":
                ...
                if output_file_id:
                    ...
                    raise RuntimeError(
                        f"Batch completed but no output file: {batch_id}"
                    )
