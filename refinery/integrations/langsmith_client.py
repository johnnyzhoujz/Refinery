...
class LangSmithClient(TraceProvider):
    ...
    def _parse_datetime(self, iso_string: str) -> datetime:
        ...
        try:
            ...
            logger.warning("Failed to parse datetime", iso_string=iso_string, error=str(e))
    ...
    async def _fetch_runs_by_query(self, query_request: LangSmithQueryRequest) -> List[LangSmithRun]:
        ...
        try:
            ...
            logger.error("Failed to fetch runs", error=str(e), query=query_request.dict())
            raise
    ...
    async def fetch_trace(self, trace_id: str) -> Trace:
        ...
        query_request = LangSmithQueryRequest(
            ...
            select=["id", "name", "run_type", "inputs", "outputs", "start_time", "end_time", 
                   "error", "parent_run_id", "trace_id", "dotted_order", "session_id", "extra"],
        ...
        logger.info("Successfully fetched trace", trace_id=trace_id, run_count=len(runs))
    ...
    async def fetch_failed_traces(
        ...
        """
        cache_key = f"failed_{project}_{start_time.isoformat()}_{end_time.isoformat()}_{limit}"
        cached_result = self.cache.get(cache_key)
        if cached_result is not None:
            logger.debug("Cache hit for failed traces", project=project)
            return cached_result
        
        logger.info("Fetching failed traces", project=project, start_time=start_time, end_time=end_time)
        ...
        query_request = LangSmithQueryRequest(
            ...
            trace_filter=f'eq(session_name, "{project}")' if project != "unknown" else None,
            select=["trace_id"],
        ...
        logger.info("Successfully fetched failed traces", project=project, count=len(traces))
    ...
    async def fetch_trace_hierarchy(self, trace_id: str) -> Dict[str, Any]:
        ...
        logger.info("Successfully built trace hierarchy", trace_id=trace_id, root_runs=len(root_nodes))
