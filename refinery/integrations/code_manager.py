...
class SafeCodeManager(CodeManager):
    ...
    async def analyze_codebase(self, path: str) -> CodeContext:
        ...
        main_language = max(language_counts.items(), key=lambda x: x[1])[0] if language_counts else 'python'
            ...
            with self._atomic_changes():
                for change in changes:
                    ...
                    with self._lock_manager.acquire(change.file_path):
                        ...
                        original_content = file_path.read_text() if original_exists else ""
    ...
    async def _extract_python_dependencies(self, path: Path) -> Dict[str, str]:
        ...
        dep_files = ['requirements.txt', 'setup.py', 'setup.cfg', 'pyproject.toml', 'Pipfile']
    ...
    def _find_config_files(self) -> List[str]:
        ...
        config_patterns = [
            ...
            "*.env", ".env.*", "config.py", "settings.py", "configuration.py"
        ]
    ...
    async def _validate_python_change(self, change: FileChange) -> Tuple[List[str], List[str]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                if node.func.id == 'print':
                    warnings.append(f"Found print statement at line {node.lineno}. Consider using logging instead.")
        
        # Check for broad exception handling
        for node in ast.walk(tree):
            if isinstance(node, ast.ExceptHandler):
                if node.type is None or (isinstance(node.type, ast.Name) and node.type.id == 'Exception'):
                    warnings.append(f"Broad exception handling at line {node.lineno}. Consider catching specific exceptions.")
    ...
    async def _analyze_python_impact(
        ...
        if 'api' in change.file_path.lower() or 'interface' in change.file_path.lower():
            breaking_changes.append(f"API change in {change.file_path} may affect external consumers")
...
class LockManager:
    ...
    def acquire(self, file_path: str, timeout: float = 10.0):
        ...
        lock_path = self.lock_dir / f"{hashlib.md5(file_path.encode()).hexdigest()}.lock"
