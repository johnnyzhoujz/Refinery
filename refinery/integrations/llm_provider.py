...
class ConfigurableLLMProvider(LLMProvider):
    ...
    def __init__(self):
        ...
        logger.info(
            "Initialized LLM provider", provider=self.provider, model=self.model
        )
    ...
    async def complete(
        ...
        try:
            ...
            logger.error("LLM completion failed", error=str(e), provider=self.provider)
            raise
...
"""
            ...
            try:
                ...
                return {"reasoning": response, "tool_calls": [], "result": response}

        except Exception as e:
            logger.error(
                "LLM tool completion failed", error=str(e), provider=self.provider
            )
            raise
