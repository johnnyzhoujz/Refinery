...
from ..core.models import DomainExpertExpectation, Trace
...
class VectorStoreManager:
    ...
    async def create_single_store_with_all_files(
        ...
        """
        logger.info(
            f"Creating single vector store for chunked analysis: trace_id={trace.trace_id}"
        )
        ...
        try:
            ...
            logger.warning(
                f"Failed to create vector store with TTL, retrying without: {e}"
            )
            vector_store = self.client.vector_stores.create(
                name=f"refinery_chunked_{trace.trace_id}_{int(datetime.now().timestamp())}"
            )
        ...
        logger.info(
            f"Added {len(file_ids)} files to chunked vector store {vector_store.id}"
        )
    ...
    def _create_trace_file(
        self, trace: Trace, expectation: DomainExpertExpectation
    ) -> str:
...
"""
    ...
    def _create_grouped_trace_files(
        self, trace: Trace, group_size: int = 6
    ) -> List[Tuple[str, str]]:
        """
        ...
        logger.info(
            f"Creating {num_groups} groups for {total_runs} runs (group_size={group_size})"
        )
...
"""
            ...
            for i, run in enumerate(group_runs, start=start_idx + 1):
                start_iso = (
                    getattr(run.start_time, "isoformat", lambda: None)() or "N/A"
                )
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            ...
            with open(f.name, "rb") as file_obj:
                file_response = self.client.files.create(
                    file=(filename, file_obj), purpose="assistants"
                )
    ...
    async def _poll_vector_store_ready(
        self, vector_store_id: str, max_wait_minutes: int = 10
    ) -> None:
        ...
        for i in range(max_iterations):
            ...
            logger.info(
                f"Vector store status: {vector_store_id} - {vector_store.status}"
            )
        ...
        raise Exception(
            f"Vector store indexing timeout after {max_wait_minutes} minutes: {vector_store_id}"
        )
