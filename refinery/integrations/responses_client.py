...
class ResponsesClient:
            ...
            try:
                ...
                for attempt in range(3):
                    if response.status_code == 429:
                        ...
                        logger.warning(
                            f"Rate limit hit, waiting {wait_time}s (attempt {attempt+1}/3)"
                        )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to parse response JSON: {response_text[:500]}"
                    )
    ...
    def parse_json_output(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if output_item.get("type") == "message":
                        ...
                        for item in output_content:
                            if item.get("type") == "output_text":
                                text = item.get("text", "")
                                logger.debug(
                                    f"[GPT5-DEBUG] Found output_text: {text[:200]}..."
                                )
                        ...
                        if text_parts:
                            ...
                            logger.info(
                                f"[GPT5-DEBUG] Successfully extracted text ({len(full_text)} chars), parsing JSON..."
                            )
            ...
            logger.error(
                f"Cannot find JSON in response structure: {json.dumps(response, indent=2)[:1000]}"
            )
    ...
    async def create_with_retry(
        self, body: Dict[str, Any], max_format_retries: int = 1
    ) -> Dict[str, Any]:
        """
    ...
    async def create_background(
        ...
        logger.info(
            f"[GPT5-DEBUG] Initial response status: {status}, id: {response_id}"
        )
        ...
        if status in {"succeeded", "completed"}:
            ...
            logger.info(
                f"[GPT5-DEBUG] Fast-complete response structure: {json.dumps(initial, indent=2)[:2000]}"
            )
            json_output, usage_total = self.parse_json_and_usage(initial)
            logger.info(
                f"[GPT5-DEBUG] Parsed JSON keys: {list(json_output.keys()) if isinstance(json_output, dict) else type(json_output)}"
            )
        ...
        while True:
            ...
            if job_status in {"succeeded", "completed"}:
                ...
                logger.info(
                    f"[GPT5-DEBUG] Polled job complete. Response structure: {json.dumps(job, indent=2)[:2000]}"
                )
...
async def create_background(
    ...
    if _client is None:
        raise Exception("Client not initialized. Call init_client() first.")
    return await _client.create_background(
        body, poll_interval=poll_interval, timeout=timeout
    )
