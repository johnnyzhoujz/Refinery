...
def get_versioned_prompt(
    ...
    if context is None:
        raise ValueError(
            "Context (globals()) must be provided to access prompt definitions"
        )
    ...
    if version is None:
        ...
        if version:
            logger.info(
                f"Using version {version} for {base_name} from env var {specific_env}"
            )
        else:
...
def list_prompt_versions(
    base_name: str, context: Optional[Dict[str, Any]] = None
) -> list:
    """
...
def migrate_prompt_to_versioned(
    module_globals: Dict[str, Any], prompt_names: list, default_version: str = "V1"
) -> None:
    """
