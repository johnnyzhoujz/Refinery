...
class RefineryContext:
    ...
    def _load_all_contexts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("version") != CONTEXT_VERSION:
                logger.debug(
                    "Context version mismatch, migrating",
    ...
    def create_or_update_context(
        ...
        project_context = all_contexts.get("projects", {}).get(
            project_name,
            {"prompt_files": [], "eval_files": [], "config_files": [], "metadata": {}},
        )
    ...
    def remove_files(
        ...
        if prompt_files:
            ...
            context["prompt_files"] = [
                f for f in context.get("prompt_files", []) if f not in normalized
            ]

        if eval_files:
            normalized = self._normalize_paths(eval_files)
            context["eval_files"] = [
                f for f in context.get("eval_files", []) if f not in normalized
            ]

        if config_files:
            normalized = self._normalize_paths(config_files)
            context["config_files"] = [
                f for f in context.get("config_files", []) if f not in normalized
            ]
    ...
    def validate_and_clean_context(self, project_name: str) -> Dict[str, Any]:
        ...
        return {"context": cleaned_context, "missing_files": all_missing}
    ...
    def store_trace_prompts(
        self, project_name: str, extracted_prompts: Dict[str, Any], trace_id: str
    ) -> Dict[str, List[str]]:
        ...
        created_files = {"prompt_files": [], "eval_files": [], "config_files": []}
        ...
        for i, prompt_data in enumerate(extracted_prompts.get("system_prompts", [])):
            ...
            created_files["prompt_files"].append(
                str(filepath.relative_to(self.codebase_path))
            )
        ...
        for i, prompt_data in enumerate(extracted_prompts.get("user_prompts", [])):
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(
                    f"# User {'Template' if has_vars else 'Prompt'} from trace {trace_id}\n"
                )
                f.write(f"# Run: {prompt_data.get('run_name', 'unknown')}\n")
        ...
        for i, template_data in enumerate(
            extracted_prompts.get("prompt_templates", [])
        ):
            ...
            filename = (
                f"template_{template_data.get('key', 'unknown')}_{content_hash}.txt"
            )
            filepath = prompts_dir / filename
        ...
        if extracted_prompts.get("model_configs"):
            ...
            created_files["config_files"].append(
                str(config_file.relative_to(self.codebase_path))
            )
        ...
        if extracted_prompts.get("eval_examples"):
                ...
                json.dump(
                    {
                        ...
                        "examples": extracted_prompts["eval_examples"][
                            :10
            ...
            created_files["eval_files"].append(
                str(eval_file.relative_to(self.codebase_path))
            )
            ...
            json.dump(
                {
                    ...
                    "extraction_summary": {
                        "system_prompts": len(
                            extracted_prompts.get("system_prompts", [])
                        ),
                        "user_prompts": len(extracted_prompts.get("user_prompts", [])),
                        "templates": len(extracted_prompts.get("prompt_templates", [])),
                        "model_configs": len(
                            extracted_prompts.get("model_configs", [])
                        ),
                        "eval_examples": len(
                            extracted_prompts.get("eval_examples", [])
                        ),
                    },
...
def load_or_create_context(
    codebase_path: str, project_name: str
