...
class AgentContextBuilder:
    ...
    async def build_from_trace(self, trace, langsmith_client) -> AgentContext:
        ...
        self.context = AgentContext(
            ...
            model_config=(
                extracted.get("model_configs", [{}])[0]
                if extracted.get("model_configs")
                else {}
            ),
    ...
    def _extract_test_cases_from_python(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(test_pattern, content):
            ...
            test_cases.append(
                {"name": test_name, "description": test_doc, "type": "unit_test"}
            )
...
class AgentContextResolver:
    ...
    async def resolve_context(
        ...
        if local_path and os.path.exists(local_path):
            ...
            logger.info(
                "Loaded context from local files",
