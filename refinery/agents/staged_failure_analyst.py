    ...
    async def _run_stage1_chunked(
        ...
        *,
        ...
        for group_idx in range(num_groups):
            ...
            self._emit_progress(
                "stage1_group_start",
                {"group_index": group_idx + 1, "total_groups": num_groups, "group_size": group_size},
            )
...
{STAGE1_TRACE_ANALYSIS_PROMPT}"""
            ...
            body = {
                ...
                "input": [
                    {"type": "message", "role": "system", "content": [{"type": "input_text", "text": FAILURE_ANALYST_SYSTEM_PROMPT_V3}]},
                    {"type": "message", "role": "user", "content": [{"type": "input_text", "text": user_message}]},
                ],
                "text": {
                    "format": {
                        "type": "json_schema",
            ...
            try:
                for retry_attempt in range(3):
                    try:
                        ...
                        usage_total = metadata.get("usage_total_tokens") if metadata else None

                        # DEBUG: Log what we got back
                        logger.info(f"[GPT5-DEBUG] Chunk {group_idx + 1} result type: {type(result)}")
                        logger.info(f"[GPT5-DEBUG] Chunk {group_idx + 1} result keys: {list(result.keys()) if isinstance(result, dict) else 'N/A'}")
                        logger.info(f"[GPT5-DEBUG] Chunk {group_idx + 1} timeline count: {len(result.get('timeline', []))}")
                        logger.info(f"[GPT5-DEBUG] Chunk {group_idx + 1} events count: {len(result.get('events', []))}")
                        logger.info(f"[GPT5-DEBUG] Chunk {group_idx + 1} evidence count: {len(result.get('evidence', []))}")
                        if len(result.get('timeline', [])) == 0:
                            logger.warning(f"[GPT5-DEBUG] Chunk {group_idx + 1} returned EMPTY timeline! Full result: {json.dumps(result, indent=2)[:1000]}")
                ...
                partials.append({"timeline": [], "events": [], "evidence": [], "coverage": {}})
                self._emit_progress(
                    "stage1_group_failed",
            ...
            if group_idx < num_groups - 1:
                ...
                self._emit_progress(
                    "stage1_group_sleep",
                    {"group_index": group_idx + 1, "completed_groups": group_idx + 1, "sleep_seconds": inter_group_sleep},
                )
    ...
    def _merge_stage1_results(self, partials: list[dict]) -> dict:
        ...
        logger.info(f"[GPT5-DEBUG] _merge_stage1_results called with {len(partials)} partials")
        for idx, partial in enumerate(partials):
            logger.info(f"[GPT5-DEBUG] Partial {idx + 1}: type={type(partial)}, keys={list(partial.keys()) if isinstance(partial, dict) else 'N/A'}")
            logger.info(f"[GPT5-DEBUG] Partial {idx + 1}: timeline={len(partial.get('timeline', []))}, events={len(partial.get('events', []))}, evidence={len(partial.get('evidence', []))}")
