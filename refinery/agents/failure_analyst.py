...
class AdvancedFailureAnalyst(FailureAnalyst):
    ...
    async def _holistic_batch_analysis(
        ...
        logger.info(
            "Starting holistic batch analysis",
        ...
        batch_id = await self._submit_holistic_batch(
            trace_data, expectation, trace.trace_id
        )
    ...
    async def analyze_trace(
        ...
        return TraceAnalysis(
            ...
            context_at_each_step={
                "summary": trace_analysis_data.get("execution_summary", "")
            },
    ...
    async def compare_to_expected(
        ...
        return GapAnalysis(
            ...
            suggested_focus_areas=gap_analysis_data.get(
                "behavioral_differences", []
            ),  # Use behavioral_differences as focus areas
        )
...
"""
        ...
        logger.info(
            "Prepared trace data for inline analysis",
    ...
    async def _submit_holistic_batch(
        self,
        trace_data: str,
        expectation: DomainExpertExpectation,
        trace_id: str = None,
        ...
        analysis_request = {
            ...
            "input": [
                ...
                {"type": "message", "role": "user", "content": combined_prompt},
            ],
        ...
        try:
            with open(batch_file_path, "rb") as f:
                batch_file_response = self.client.files.create(file=f, purpose="batch")
        ...
        while True:
            ...
            if status == "completed":
                ...
                if successful_count > 0 and fresh_batch.output_file_id:
                    return await self._download_batch_results(
                        fresh_batch.output_file_id
                    )
                    ...
                    raise RuntimeError(
                        f"All batch requests failed. Error: {error_text}"
                    )
                else:
                    raise RuntimeError(
                        f"Batch completed but no output or error file: {batch_id}"
                    )
