...
from refinery.interfaces.chat_interface import BaseChatInterface, ChatInterface
from refinery.interfaces.chat_session import run_chat_session
from refinery.core.models import Diagnosis, FailureType, Confidence
...
class MockNLInterface(BaseChatInterface):
    ...
    async def show_diagnosis(self, diagnosis):
        ...
        print(f"🤖 This looks like a {diagnosis.failure_type.value.replace('_', ' ')} problem.")
...
async def demo_mock_nl_interface():
    ...
    expected = await interface.get_expected_behavior()
    ...
    if await interface.confirm_action("Should I apply a fix?"):
        await interface.show_success("Fix applied! Added memory limitation instructions to system prompt.")
